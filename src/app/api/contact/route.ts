...
export async function POST(request: Request) {
  try {
    ...
    const { name, email, role, message, phone, source, 'bot-field': botField } = body;
    ...
    const subject = isRoadmap
      ? `[The Digital Sanctuary] Roadmap Consultation Request: ${name}`
      : `[The Digital Sanctuary] New Lead: ${name}`;
    ...
    await transporter.sendMail({
      ...
      html: `
        <div dir="rtl" style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
          <h2 style="color:#3D3530;margin-bottom:8px">${isRoadmap ? 'בקשה למפת דרכים' : 'פנייה חדשה מהאתר'}</h2>
          ${isRoadmap ? '<p style="color:#6B8C6B;font-size:13px;margin-bottom:24px">מגיע מדף /roadmap</p>' : ''}
          <table style="width:100%;border-collapse:collapse">
            ...
            ${phone ? `<tr>
              <td style="padding:8px 0;color:#6B5E52;font-weight:bold">טלפון:</td>
              <td style="padding:8px 0"><a href="tel:${phone}" style="color:#6B8C6B">${phone}</a></td>
            </tr>` : ''}
            <tr>
